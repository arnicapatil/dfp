...
$ruserid=filter_input(INPUT_POST,'userid');
$rsub=filter_input(INPUT_POST,'st');
            ...
            if(!empty($rphone)){
                if(!empty($ruserid)){
                    if(!empty($rsub)){
                                    ...
                                    if(!empty($rp1pin)){
                                        if(!empty($rp1add)){
                                            ...
                                            if(mysqli_connect_error()){
                                                ...
                                                $sql = "INSERT INTO register (r_firstname, r_lastname, r_email, r_phone, user_id, sub, r_p1name, r_p2name, r_p1gender, r_p2gender, r_p1dob, r_p2dob, r_p1pincode, r_p2pincode, r_p1address, r_p2address)
                                                values ('$rfirstn','$rlastn','$remail','$rphone','$ruserid','$rsub','$rp1name', '$rp2name', '$rp1g', '$rp2g', '$rp1dob', '$rp2dob', '$rp1pin', '$rp2pin', '$rp1add', '$rp2add')";
                                                if ($conn->query($sql)){
                        ...
                        echo "Subscription Type cant be empty!";
                        die();
                    }
                }
                else{
